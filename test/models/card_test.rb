...
class CardTest < ActiveSupport::TestCase
  ...
  test "should require session_id" do
    ...
    assert_includes card.errors[:session_id], "não pode ficar em branco"
  end

  test "should require grid_data" do
    card = Card.new(game: games(:one), session_id: "session123")
    assert_not card.valid?
    assert_includes card.errors[:grid_data], "não pode ficar em branco"
  end
  ...
  test "should validate FREE at center" do
    ...
    assert_not card.valid?
    assert_includes card.errors[:grid_data], "célula central [2][2] deve ser :free"
  end
